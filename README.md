...
- **现代管理工作台**：AD 目录、账号创建、回收、审计与计划任务使用统一的浅色操作界面
...
项目根目录附带当前 macOS Apple Silicon（arm64）版本的 `ad-server` 二进制；无需安装 Go 环境即可运行。其他操作系统或架构请按下方命令交叉编译：
...
> ```
> `-ldflags="-s -w"` 去除调试信息可减小体积约 30%；本项目使用纯 Go SQLite（无 CGO），可生成单一静态二进制。运行二进制时请保留同级 `frontend/` 目录，以便服务加载页面资源。
